...
def main(args):
    ...
    # model = create_model('deit_base_patch16_224', pretrained=False, num_classes=args.num_classes).to(device)
    ...
    for epoch in range(start_epoch + 1, args.epochs + 1):
        ...
        if val_acc > best_acc:
            ...
            old_best_model_path = os.path.join("./model_weight", "best_model_epoch{}.pth".format(epoch - 1))
            if os.path.exists(old_best_model_path):
                os.remove(old_best_model_path)
            torch.save(model.state_dict(), "./model_weight/best_model_epoch{}.pth".format(epoch))
            print("Saved epoch{} as new best model with accuracy: {:.3f}".format(epoch, val_acc))

        metrics_file = "./metrics/HyperMamba_base.metrics.csv"
