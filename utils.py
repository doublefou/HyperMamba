...
def evaluate(model, data_loader, device, epoch,num_classes):
    ...
    for step, data in enumerate(data_loader):
        ...
        accu_num += torch.eq(pred_classes, labels.to(device)).sum().float()
    ...
    print(f"[验证期 {epoch}] 损失: {avg_loss:.3f}, 准确率: {avg_accuracy:.3f}, "
          f"精度: {precision:.3f}, 召回率: {recall:.3f}, F1 分数: {f1_score:.3f}")

    return avg_loss, avg_accuracy, precision, recall, f1_score
